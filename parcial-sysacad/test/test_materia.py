...
class MateriaTestCase(unittest.TestCase):
    ...
    def test_actualizar(self):
        ...
        actualizado = MateriaService.actualizar(materia.id, datos_actualizacion)
    ...
    def test_asociar_y_desasociar_autoridad(self):
        ...
        # Asociar autoridad
        resultado = MateriaService.asociar_autoridad(materia.id, autoridad.id)
        ...
        resultado = MateriaService.desasociar_autoridad(materia.id, autoridad.id)
        self.assertTrue(resultado)
        materia_actualizada = MateriaService.buscar_por_id(materia.id)
        self.assertNotIn(autoridad, materia_actualizada.autoridades)

    def test_relacion_autoridades(self):
        materia = nuevamateria()
        autoridad1 = nuevaautoridad(nombre="Autoridad 1")
        autoridad2 = nuevaautoridad(nombre="Autoridad 2")

        # Asociar autoridades
        materia.autoridades.append(autoridad1)
        materia.autoridades.append(autoridad2)
        db.session.commit()

        # Recargar desde BD
        materia_db = MateriaService.buscar_por_id(materia.id)
        self.assertIn(autoridad1, materia_db.autoridades)
        self.assertIn(autoridad2, materia_db.autoridades)